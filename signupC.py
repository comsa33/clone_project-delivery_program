...
class Login_info(QMainWindow, login_info_sub): # 회원가입 창
    ...
    def __init__(self):
        ...
        try:
            with open("log_c.pickle",'rb')as r:
                self.login_c=pickle.load(r)
        except FileNotFoundError:
            pass

    def initUI(self):
        self.setWindowTitle('B달의 민족_본사용 회원가입')
        self.resize(372, 250)
        self.center()
    ...
    def check(self):                              #중복확인 함수
        if self.id_info.text() in self.login_c.keys():
    ...
    def loginfo(self):
        ...
        if buttonReply == QMessageBox.Yes:
            ...
            # id 키값 / 나머지 value
            self.login_c[self.id_info.text()] = [self.password_info.text(), self.name_info.text(),
                                               self.address_info.text(), self.ph_num_info.text()]

            with open("log_c.pickle", 'wb')as f:
                pickle.dump(self.login_c, f)
